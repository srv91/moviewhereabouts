...
latest = RT(my_key).lists('movies', 'opening', page=2)

print "Getting opening movies..."
for movie in latest['movies']:
    if Movie.objects.filter(id = movie['id']).exists()==False:
        try:
        
            m = models.Movie(id = movie['id'], title = movie['title'], movie_synopsis = movie['synopsis'], critics_score = movie['ratings']['critics_score'], release = movie['year'], votes = 10, img = movie['title'].replace(" ","").lower(), category = 'latest')
            m.save()
